...
public class MedumoneMixin {
    ...
    public int getMaxMana() {
        return ConfigFile.HANDLER.instance().getFunctional().getMedumone().manaCapacity;
    }
    ...
    public int modifyCost1(MedumoneBlockEntity instance) {
        return instance.getMana() - ConfigFile.HANDLER.instance().getFunctional().getMedumone().manaCost;
    }
    ...
    public void modifyCost2(MedumoneBlockEntity instance, int i) {
        instance.addMana(-ConfigFile.HANDLER.instance().getFunctional().getMedumone().manaCost);
    }
    ...
    public int modifyCost3(MedumoneBlockEntity instance) {
        return instance.getMana() < ConfigFile.HANDLER.instance().getFunctional().getMedumone().manaCost ? 0 : instance.getMana();
    }
    ...
    private BlockPos configureRange1(BlockPos instance, int dx, int dy, int dz) {
        final int RANGE = ConfigFile.HANDLER.instance().getFunctional().getMedumone().range;
    ...
    private int configureRange3(int range) {
        return ConfigFile.HANDLER.instance().getFunctional().getMedumone().range;
    }
}
